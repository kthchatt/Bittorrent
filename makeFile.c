...
int createTorrent(Torrent t){
	...
	fprintf(file, "d8:announce%lu:%s13:announce-listll", strlen(t.announce), t.announce);
	for(i=0; i<sizeof(t.announceList)/sizeof(t.announceList[0])-1; i++)
		fprintf(file, "%lu:%sel", strlen(t.announceList[i]), t.announceList[i]);
	fprintf(file, "%lu:%see4:infod5:filesld", strlen(t.announceList[i]), t.announceList[i]);
	
	for(i=0; i<sizeof(t.files)/sizeof(t.files[0]); i++)
		fprintf(file, "6:lengthi%de4:name%lu:%s", t.files[i].length, strlen(t.files[i].name), t.files[i].name);
	
	fprintf(file, "12:piece lengthi%de6:pieces%d:", t.pieceLength, sizeof(t.pieces)/t.pieceLength);
